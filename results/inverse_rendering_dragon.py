### Inverse rendering optimization for the mixture weight of a dragon's diffuse/specular BRDF
...
lr = 0.01
spp = 3; spp_grad = 1; spp_label = f'{spp} primal, {spp_grad} adjoint'
num_iterations = 100
height = 384; width = 216
scene_name = "Dragon"; scene_key = 'dragon'
...
alpha_ref = 0.1; alpha_init = 0.7
out_path = "out/inverse-rendering/dragon/"
...
def run_optimization(differential_sampling, adaptive_sampling):
    ...
    params_ref['diff_bsdf.bsdf_1.alpha.value'] = 0.001
    params_ref[key] = alpha_ref; params_ref.update();
    integrator_bsdf = mi.load_dict({'type': 'prb', 'max_depth': max_depth, 'rr_depth': rr_depth})
    img_ref = dr.detach(mi.render(scene_ref, params_ref, integrator=integrator_bsdf, spp=ref_spp, seed=10001))
    ...
    params['diff_bsdf.bsdf_1.alpha.value'] = 0.001
    params[key] = alpha_init; params.update();
    img = mi.render(scene, params, integrator=integrator_bsdf, spp=ref_spp, seed=20001)
